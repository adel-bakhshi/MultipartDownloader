...
public class RequestConfiguration
{
    ...
    /// <summary>
    /// Set <see cref="HttpWebRequest.AllowAutoRedirect"/> to true to allow the current request to automatically
    ...
    /// <see cref="HttpWebRequest.MaximumAutomaticRedirections"/> property.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// The <seealso cref="HttpWebRequest.Credentials"/> property contains authentication
    /// information to identify the client making the request. The <see cref="HttpWebRequest.Credentials"/>
    /// property
    ...
    /// <summary>
    /// A <see cref="DateTime"/> that contains the contents of the HTTP If-Modified-Since header.
    ...
    /// <summary>
    /// An application uses <see cref="HttpWebRequest.KeepAlive"/> to
    ...
    /// If <see cref="HttpWebRequest.Pipelined"/> is true,
    /// an application makes pipelined connections to servers that support them. The default is true.
    /// Pipelined connections are made only when the <seealso cref="HttpWebRequest.KeepAlive"/> property is
    /// true.
    /// </summary>
    ...
    /// <summary>
    /// A <see cref="Version"/> that represents the HTTP version to use for the request.
    /// The default is <see cref="HttpVersion.Version10"/>.
    /// The <seealso cref="HttpWebRequest"/> class supports only versions 1.0 and 1.1 of HTTP.
    /// Setting <see cref="HttpWebRequest.ProtocolVersion"/> to a different version
    ...
    /// <summary>
    /// The <see cref="HttpWebRequest.Proxy"/> property identifies
    /// the WebProxy instance to use to communicate with the destination server.
    /// To specify that no proxy should be used, set the <see cref="HttpWebRequest"/>.
    ...
    /// Clearing <see cref="HttpWebRequest.TransferEncoding"/> by setting it to null has no effect
    /// on the value of <see cref="HttpWebRequest.SendChunked"/>.
    /// Values assigned to the <see cref="HttpWebRequest.TransferEncoding"/> property replace any existing contents.
    /// For additional information see section 14.41 of IETF RFC 2616 - HTTP/1.1.
    /// </summary>
    /// <exception cref="InvalidOperationException">
    ///     <see cref="HttpWebRequest.TransferEncoding"/> is set when
    ///     <see cref="HttpWebRequest.SendChunked"/> is false.
    /// </exception>
    /// <exception cref="ArgumentException">
    ///     <see cref="HttpWebRequest.TransferEncoding"/> is set to the value "Chunked". This value is case-insensitive.
    /// </exception>
